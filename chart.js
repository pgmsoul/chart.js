...
var Chart = function(context){
	...
	var Pie = function(data,config,ctx){
		...
		function format_num(i,n){
			i = i.toString(16);
			if(i.length>n) return i.substr(0,n);
			while(i.length<n){
				i = "0" + i;
			}
			return i;
		...
		function rgbToColor(rgb){
			var r = format_num(rgb.r,2);
			var g = format_num(rgb.g,2);
			var b = format_num(rgb.b,2);
			return "#" + r + g + b;
		}
		function get_rgb_dif(v){
			return v>=128?(v-128):(v+128);
		}
		function get_text_color(color){
			var rgb = colorToRgb(color);
			var rgb2 = {};
			rgb2.r = get_rgb_dif(rgb.r);
			rgb2.g = get_rgb_dif(rgb.g);
			rgb2.b = get_rgb_dif(rgb.b);
			return rgbToColor(rgb2);
		}
		function drawPieSegments (animationDecimal){
			...
			for (var i = 0; i < data.length; i++) {
				var d = data[i];
				var segmentAngle = rotateAnimation * ((d.value / segmentTotal) * (Math.PI * 2));
				
				var text = d.text;
				if(text===undefined){
					if(!text) text = d.label;
				}
				if(!text) continue;
				...
				ctx.fillStyle = get_text_color(d.color);
